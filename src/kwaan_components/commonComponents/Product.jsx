import { Flex, Image, Badge, Text, Button, Icon } from "@chakra-ui/react";
import { AiFillStar } from "react-icons/ai";
import AddToCart from "./AddToCart";
import { Tooltip } from "@/components/ui/tooltip";

const Product = ({ name, imgSrc, price, productStatus, titleLength, rating, addToCart }) => {
    return (
        <Tooltip showArrow contentProps={{ css: { "--tooltip-bg": "gray" } }} content={name}>
            <Flex
                direction="column"
                gap={3}
                p={{ base: 1, sm: 2, lg: 4 }}
                ...
                <Flex direction="column" justify="space-between" gap={2} h="100%">
                    <Flex justify="space-between" align="center">
                        <Text fontWeight="600" fontSize="md" color="gray.800">
                            {name.length <= titleLength ? name : `${name.slice(0, titleLength)}...`}
                        </Text>
